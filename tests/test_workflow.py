...
from app.agent.nodes.local_search import (
    local_search,
    random_initiatives,
)
...
def run_workflow(user_input, exclude_names=None, exclude_urls=None):
    ...
    if not intent.get("is_relevant", True):
        ...
        return {
            "type": "clarification",
            "message": (
                "BeKind is focused on helping people, animals, "
                "and community initiatives in Belgrade."
            )
        }
    ...
    if intent.get("category") == "random_good_deed":

        items = random_initiatives()
