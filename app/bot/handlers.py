...
async def handle_message(
    ...
    try:
        ...
        if user_text == "🔁 Suggest another":
            await _handle_suggest_another(
                update,
                context,
            )
        ...
        if user_text == "✅ I've picked a good deed":
            context.user_data.pop("shown_names", None)
            context.user_data.pop("shown_web_urls", None)
        ...
        result = run_workflow(
            user_text,
            exclude_urls=list(
                context.user_data.get("shown_web_urls", [])
            ),
            user_id=user_id,
        )
        ...
        if (
            isinstance(result, dict)
            and result.get("type") == "random_deeds"
        ):
            items = result["data"]
        ...
        if (
            isinstance(result, dict)
            and result.get("type") == "clarification"
        ):
            await update.message.reply_text(
                result["message"],
                reply_markup=main_keyboard(),
